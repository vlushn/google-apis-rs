...
use google_workflows1::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    ...
    async fn _projects_locations_workflows_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::Workflow = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_workflows_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "workflow-id" => {
    ...
    async fn _projects_locations_workflows_delete(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_workflows_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_workflows_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_workflows_get(opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_workflows_list(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_workflows_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "page-token" => {
    ...
    async fn _projects_locations_workflows_patch(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        let mut request: api::Workflow = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_workflows_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "update-mask" => {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-workflows-create", Some(opt)) => {
                        call_result = self._projects_locations_workflows_create(opt, dry_run, &mut err).await;
                    },
                    ("locations-workflows-delete", Some(opt)) => {
                        call_result = self._projects_locations_workflows_delete(opt, dry_run, &mut err).await;
                    },
                    ("locations-workflows-get", Some(opt)) => {
                        call_result = self._projects_locations_workflows_get(opt, dry_run, &mut err).await;
                    },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "workflows1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/workflows1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-list', 'locations-operations-delete', 'locations-operations-get', 'locations-operations-list', 'locations-workflows-create', 'locations-workflows-delete', 'locations-workflows-get', 'locations-workflows-list' and 'locations-workflows-patch'", vec![
            ...
            ("locations-workflows-create",
                    Some(r##"Creates a new workflow. If a workflow with the specified name already exists in the specified project and location, the long running operation will return ALREADY_EXISTS error."##),
            ...
            ("locations-workflows-get",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Name of the workflow which information should be retrieved. Format: projects/{project}/locations/{location}/workflows/{workflow}"##),
                     Some(true),
                     Some(false)),
            ...
            ("locations-workflows-patch",
                    Some(r##"Updates an existing workflow. Running this method has no impact on already running executions of the workflow. A new revision of the workflow may be created as a result of a successful update operation. In that case, such revision will be used in new workflow executions."##),
    ...
    let mut app = App::new("workflows1")
           ...
           .about("Orchestrate Workflows consisting of Google Cloud APIs, SaaS APIs or private API endpoints.")
