...
use google_videointelligence1::{api, Error, oauth2};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::CloudVideoIntelligence,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    async fn _operations_projects_locations_operations_cancel(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _operations_projects_locations_operations_delete(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _operations_projects_locations_operations_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _videos_annotate(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    "video-context.face-detection-config.include-attributes" => Some(("videoContext.faceDetectionConfig.includeAttributes", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    "video-context.face-detection-config.include-bounding-boxes" => Some(("videoContext.faceDetectionConfig.includeBoundingBoxes", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    "video-context.face-detection-config.model" => Some(("videoContext.faceDetectionConfig.model", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    ...
                    "video-context.person-detection-config.include-attributes" => Some(("videoContext.personDetectionConfig.includeAttributes", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    "video-context.person-detection-config.include-bounding-boxes" => Some(("videoContext.personDetectionConfig.includeBoundingBoxes", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    "video-context.person-detection-config.include-pose-landmarks" => Some(("videoContext.personDetectionConfig.includePoseLandmarks", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["audio-tracks", "diarization-speaker-count", "enable-automatic-punctuation", "enable-speaker-diarization", "enable-word-confidence", "explicit-content-detection-config", "face-detection-config", "features", "filter-profanity", "frame-confidence-threshold", "include-attributes", "include-bounding-boxes", "include-pose-landmarks", "input-content", "input-uri", "label-detection-config", "label-detection-mode", "language-code", "language-hints", "location-id", "max-alternatives", "model", "object-tracking-config", "output-uri", "person-detection-config", "shot-change-detection-config", "speech-transcription-config", "stationary-camera", "text-detection-config", "video-confidence-threshold", "video-context"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
        ...
        let mut request: api::GoogleCloudVideointelligenceV1_AnnotateVideoRequest = json::value::from_value(object).unwrap();
            ...
            ("operations", Some(opt)) => {
                match opt.subcommand() {
                    ("projects-locations-operations-cancel", Some(opt)) => {
                        call_result = self._operations_projects_locations_operations_cancel(opt, dry_run, &mut err).await;
                    },
                    ("projects-locations-operations-delete", Some(opt)) => {
                        call_result = self._operations_projects_locations_operations_delete(opt, dry_run, &mut err).await;
                    },
                    ("projects-locations-operations-get", Some(opt)) => {
                        call_result = self._operations_projects_locations_operations_get(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        ...
        ).persist_tokens_to_disk(format!("{}/videointelligence1", config_dir)).build().await.unwrap();
