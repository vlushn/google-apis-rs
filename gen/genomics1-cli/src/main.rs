...
use google_genomics1::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "genomics1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/genomics1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("operations", "methods: 'cancel', 'get' and 'list'", vec![
            ...
            ("get",
                    Some(r##"Gets the latest state of a long-running operation. Clients can use this method to poll the operation result at intervals as recommended by the API service. Authorization requires the following [Google IAM](https://cloud.google.com/iam) permission: * `genomics.operations.get`"##),
            ...
            ("list",
                    Some(r##"Lists operations that match the specified filter in the request. Authorization requires the following [Google IAM](https://cloud.google.com/iam) permission: * `genomics.operations.list`"##),
