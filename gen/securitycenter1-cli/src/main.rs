        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["canonical-name", "category", "create-time", "event-time", "external-uri", "marks", "name", "parent", "resource-name", "security-marks", "severity", "state"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
...
async fn main() {
    ...
    let arg_data = [
        ("folders", "methods: 'assets-group', 'assets-list', 'assets-update-security-marks', 'sources-findings-group', 'sources-findings-list', 'sources-findings-patch', 'sources-findings-set-state', 'sources-findings-update-security-marks' and 'sources-list'", vec![
            ("assets-group",
        ...
        ("projects", "methods: 'assets-group', 'assets-list', 'assets-update-security-marks', 'sources-findings-group', 'sources-findings-list', 'sources-findings-patch', 'sources-findings-set-state', 'sources-findings-update-security-marks' and 'sources-list'", vec![
            ("assets-group",
                    Some(r##"Filters an organization's assets and groups them by their specified properties."##),
