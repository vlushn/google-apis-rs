...
use google_adexchangebuyer2_v2_beta1::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    ...
    async fn _bidders_accounts_filter_sets_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "is-transient" => {
                    call = call.is_transient(arg_from_str(value.unwrap_or("false"), err, "is-transient", "boolean"));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
