...
impl<'n> Engine<'n> {
    async fn _projects_profiles_create(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    "deployment.labels" => Some(("deployment.labels", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Map })),
                    "deployment.project-id" => Some(("deployment.projectId", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "deployment.target" => Some(("deployment.target", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "profile-type" => Some(("profileType", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Vec })),
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["deployment", "labels", "profile-type", "project-id", "target"]);
        ...
        let mut request: api::CreateProfileRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().profiles_create(request, opt.value_of("parent").unwrap_or(""));
    ...
    async fn _projects_profiles_create_offline(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut call = self.hub.projects().profiles_create_offline(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_profiles_patch(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::Profile = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().profiles_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "update-mask" => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "cloudprofiler2-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/cloudprofiler2", config_dir)).build().await.unwrap();
