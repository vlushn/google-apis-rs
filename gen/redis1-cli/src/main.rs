...
impl<'n> Engine<'n> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    "alternative-location-id" => Some(("alternativeLocationId", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    "auth-enabled" => Some(("authEnabled", JsonTypeInfo { jtype: JsonType::Boolean, ctype: ComplexType::Pod })),
                    ...
                    "memory-size-gb" => Some(("memorySizeGb", JsonTypeInfo { jtype: JsonType::Int, ctype: ComplexType::Pod })),
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["alternative-location-id", "auth-enabled", "authorized-network", "connect-mode", "create-time", "current-location-id", "display-name", "host", "labels", "location-id", "memory-size-gb", "name", "persistence-iam-identity", "port", "redis-configs", "redis-version", "reserved-ip-range", "state", "status-message", "tier", "transit-encryption-mode"]);
    ...
    async fn _projects_locations_instances_export(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _projects_locations_instances_failover(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    "data-protection-mode" => Some(("dataProtectionMode", JsonTypeInfo { jtype: JsonType::String, ctype: ComplexType::Pod })),
                    _ => {
        ...
        let mut request: api::FailoverInstanceRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_instances_failover(request, opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_instances_get_auth_string(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_instances_get_auth_string(opt.value_of("name").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_instances_import(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-instances-export", Some(opt)) => {
                        call_result = self._projects_locations_instances_export(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-failover", Some(opt)) => {
                        call_result = self._projects_locations_instances_failover(opt, dry_run, &mut err).await;
                    },
                    ...
                    ("locations-instances-get-auth-string", Some(opt)) => {
                        call_result = self._projects_locations_instances_get_auth_string(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-import", Some(opt)) => {
                        call_result = self._projects_locations_instances_import(opt, dry_run, &mut err).await;
                    },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "redis1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/redis1", config_dir)).build().await.unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-instances-create', 'locations-instances-delete', 'locations-instances-export', 'locations-instances-failover', 'locations-instances-get', 'locations-instances-get-auth-string', 'locations-instances-import', 'locations-instances-list', 'locations-instances-patch', 'locations-instances-upgrade', 'locations-list', 'locations-operations-cancel', 'locations-operations-delete', 'locations-operations-get' and 'locations-operations-list'", vec![
            ("locations-get",
