...
use google_jobs4::{api, Error, oauth2};
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["allow-imprecise-addresses", "allow-missing-ids", "commute-filter", "commute-method", "companies", "company-display-names", "compensation-filter", "currency-code", "custom-attribute-filter", "custom-ranking-info", "departure-time", "device-info", "device-type", "disable-keyword-match", "disable-spell-check", "diversification-level", "domain", "employment-types", "enable-broadening", "end-time", "excluded-jobs", "hours", "id", "importance-level", "include-jobs-with-unspecified-compensation-range", "job-categories", "job-query", "job-view", "language-codes", "latitude", "longitude", "max-compensation", "max-page-size", "min-compensation", "minutes", "nanos", "offset", "order-by", "page-token", "publish-time-range", "query", "query-language-code", "range", "ranking-expression", "request-metadata", "road-traffic", "search-mode", "seconds", "session-id", "start-coordinates", "start-time", "travel-duration", "type", "units", "user-id"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
