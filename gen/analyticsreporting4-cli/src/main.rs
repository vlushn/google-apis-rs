...
use google_analyticsreporting4::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    async fn _reports_batch_get(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _user_activity_search(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        match self.opt.subcommand() {
            ("reports", Some(opt)) => {
                match opt.subcommand() {
                    ("batch-get", Some(opt)) => {
                        call_result = self._reports_batch_get(opt, dry_run, &mut err).await;
                    },
                    _ => {
            ...
            ("user-activity", Some(opt)) => {
                match opt.subcommand() {
                    ("search", Some(opt)) => {
                        call_result = self._user_activity_search(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "analyticsreporting4-secret.json",
