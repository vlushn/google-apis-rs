...
impl<'n> Engine<'n> {
    ...
    async fn _projects_locations_instances_namespaces_get_iam_policy(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        let mut call = self.hub.projects().locations_instances_namespaces_get_iam_policy(opt.value_of("resource").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                "options-requested-policy-version" => {
    ...
    async fn _projects_locations_instances_namespaces_set_iam_policy(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut call = self.hub.projects().locations_instances_namespaces_set_iam_policy(request, opt.value_of("resource").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_instances_namespaces_test_iam_permissions(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut call = self.hub.projects().locations_instances_namespaces_test_iam_permissions(request, opt.value_of("resource").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
    ...
    async fn _projects_locations_remove_iam_policy(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut call = self.hub.projects().locations_remove_iam_policy(request, opt.value_of("resource").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-instances-namespaces-get-iam-policy", Some(opt)) => {
                        call_result = self._projects_locations_instances_namespaces_get_iam_policy(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-namespaces-set-iam-policy", Some(opt)) => {
                        call_result = self._projects_locations_instances_namespaces_set_iam_policy(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-namespaces-test-iam-permissions", Some(opt)) => {
                        call_result = self._projects_locations_instances_namespaces_test_iam_permissions(opt, dry_run, &mut err).await;
                    },
                    ...
                    ("locations-remove-iam-policy", Some(opt)) => {
                        call_result = self._projects_locations_remove_iam_policy(opt, dry_run, &mut err).await;
                    },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "datafusion1-beta1-secret.json",
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-instances-create', 'locations-instances-delete', 'locations-instances-get', 'locations-instances-get-iam-policy', 'locations-instances-list', 'locations-instances-namespaces-get-iam-policy', 'locations-instances-namespaces-set-iam-policy', 'locations-instances-namespaces-test-iam-permissions', 'locations-instances-patch', 'locations-instances-restart', 'locations-instances-set-iam-policy', 'locations-instances-test-iam-permissions', 'locations-instances-upgrade', 'locations-list', 'locations-operations-cancel', 'locations-operations-delete', 'locations-operations-get', 'locations-operations-list', 'locations-remove-iam-policy' and 'locations-versions-list'", vec![
            ("locations-get",
            ...
            ("locations-instances-upgrade",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Name of the Data Fusion instance which need to be upgraded in the form of projects/{project}/locations/{location}/instances/{instance} Instance will be upgraded with the latest stable version of the Data Fusion."##),
                     Some(true),
                     Some(false)),
            ...
            ("locations-remove-iam-policy",
                  ...
                  vec![
                    (Some(r##"resource"##),
                     None,
                     Some(r##"The resource on which IAM policy to be removed is attached to."##),
                     Some(true),
                     Some(false)),
