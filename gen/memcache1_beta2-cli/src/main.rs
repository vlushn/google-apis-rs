...
use google_memcache1_beta2::{api, Error, oauth2};
...
struct Engine<'n> {
    opt: ArgMatches<'n>,
    hub: api::CloudMemorystoreForMemcached,
    gp: Vec<&'static str>,
...
impl<'n> Engine<'n> {
    ...
    async fn _projects_locations_instances_apply_parameters(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
    ...
    async fn _projects_locations_instances_apply_software_update(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
        ...
        let mut request: api::ApplySoftwareUpdateRequest = json::value::from_value(object).unwrap();
        let mut call = self.hub.projects().locations_instances_apply_software_update(request, opt.value_of("instance").unwrap_or(""));
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            let (key, value) = parse_kv_arg(&*parg, err, false);
            match key {
                _ => {
        ...
        for kvarg in opt.values_of("kv").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> =
                match &temp_cursor.to_string()[..] {
                    ...
                    _ => {
                        let suggestion = FieldCursor::did_you_mean(key, &vec!["authorized-network", "cpu-count", "create-time", "discovery-endpoint", "display-name", "id", "labels", "memcache-full-version", "memcache-version", "memory-size-mb", "name", "node-config", "node-count", "parameters", "params", "state", "update-available", "update-time", "zones"]);
                        err.issues.push(CLIError::Field(FieldError::Unknown(temp_cursor.to_string(), suggestion, value.map(|v| v.to_string()))));
                        None
                    }
                };
    ...
    async fn _projects_locations_instances_update_parameters(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
            ...
            ("projects", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("locations-instances-apply-parameters", Some(opt)) => {
                        call_result = self._projects_locations_instances_apply_parameters(opt, dry_run, &mut err).await;
                    },
                    ("locations-instances-apply-software-update", Some(opt)) => {
                        call_result = self._projects_locations_instances_apply_software_update(opt, dry_run, &mut err).await;
                    },
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "memcache1-beta2-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/memcache1-beta2", config_dir)).build().await.unwrap();
