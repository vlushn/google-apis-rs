...
use google_mybusiness4::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    ...
    async fn _accounts_locations_patch(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "attribute-mask" => {
                    call = call.attribute_mask(value.unwrap_or(""));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                ...
                "answers-per-question" => {
                    call = call.answers_per_question(arg_from_str(value.unwrap_or("-0"), err, "answers-per-question", "integer"));
                },
                _ => {
                    let mut found = false;
                    for param in &self.gp {
    ...
    async fn _chains_search(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
        ...
        for parg in opt.values_of("v").map(|i|i.collect()).unwrap_or(Vec::new()).iter() {
            ...
            match key {
                "result-count" => {
                    call = call.result_count(arg_from_str(value.unwrap_or("-0"), err, "result-count", "integer"));
                },
                "chain-display-name" => {
