...
use google_plus1::{api, Error, oauth2};
...
impl<'n> Engine<'n> {
    ...
    async fn _activities_search(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["language", "max-results", "order-by", "page-token"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
    ...
    async fn _people_search(&self, opt: &ArgMatches<'n>, dry_run: bool, err: &mut InvalidOptionsError)
                                                    -> Result<(), DoitError> {
                ...
                _ => {
                    ...
                    if !found {
                        err.issues.push(CLIError::UnknownParameter(key.to_string(),
                                                                  {let mut v = Vec::new();
                                                                           v.extend(self.gp.iter().map(|v|*v));
                                                                           v.extend(["language", "max-results", "page-token"].iter().map(|v|*v));
                                                                           v } ));
                    }
                }
            }
        ...
        match self.opt.subcommand() {
            ("activities", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("search", Some(opt)) => {
                        call_result = self._activities_search(opt, dry_run, &mut err).await;
                    },
                    _ => {
            ...
            ("people", Some(opt)) => {
                match opt.subcommand() {
                    ...
                    ("search", Some(opt)) => {
                        call_result = self._people_search(opt, dry_run, &mut err).await;
                    },
                    _ => {
    ...
    async fn new(opt: ArgMatches<'n>) -> Result<Engine<'n>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match client::application_secret_from_directory(&config_dir, "plus1-secret.json",
        ...
        ).persist_tokens_to_disk(format!("{}/plus1", config_dir)).build().await.unwrap();
